...
export class NoirServer extends Noir {
  async compile() {
    ...
    initialiseResolver((id: any) => {
      try {
        const code = fs.readFileSync(`circuits/src/${id}`, {
          encoding: "utf-8",
        }) as string;
        return code;
      } catch (err) {
    ...
    [this.prover, this.verifier] = await setup_generic_prover_and_verifier(
      this.acir
    );
  }
  ...
  getSmartContract() {
    ...
    if (fs.existsSync(path.join(__dirname, "../../contracts/plonk_vk.sol"))) {
      fs.unlinkSync(path.join(__dirname, "../../contracts/plonk_vk.sol"));
    }

    // We write the contract to a file called 'plonk_vk.sol' in the 'contract' folder.
    fs.writeFileSync(path.join(__dirname, "../../contracts/plonk_vk.sol"), sc, {
      flag: "w",
    });
